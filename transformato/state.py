...
class IntermediateStateFactory(object):
    ...
    def _check_hmr(self, file):
        ...
        if self.configuration["simulation"].get("HMR", False):
            # Hydrogen mass (in Da) for repartitioning. Configurable via
            # `simulation.hydrogen_mass`; ~3.024 is the convention for a 4 fs
            # timestep, while the historical default of 1.5 only supports ~2.5 fs.
            hydrogen_mass = self.configuration["simulation"].get("hydrogen_mass", 1.5)
            with open(file, "r") as f:
                with open(f"{file}_tmp", "w+") as g:
                    for line in f.readlines():
                        # The run scripts build the system with `top.createSystem(...)`
                        # (both the CHARMM and Amber branches); inject the HMR option
                        # before that call, preserving the line's indentation.
                        if ".createSystem(" in line:
                            indent = line[: len(line) - len(line.lstrip())]
                            g.write(f"{indent}# Adding HMR\n")
                            g.write(
                                f"{indent}nboptions['hydrogenMass'] = {hydrogen_mass} * unit.amu\n"
                            )
                            g.write(line)
                        else:
                            g.write(line)
